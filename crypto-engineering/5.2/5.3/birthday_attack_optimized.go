...
var nBits = flag.Int("n", 8, "Number of bits. Must be multiple of 8 between 8 and 48 inclusive.")
...
func birthdayAttack(h hash.Hash) (item1, item2, digestOut []byte) {
	...
	tableSize := uint(24) // in bits
	//tableIndexMask := ^uint64(0) >> tableSize
	tableIndexMask := (uint64(2) << tableSize) - 1
	tries := make([]try, 2<<tableSize)
	//digestBytes := h.Size()
